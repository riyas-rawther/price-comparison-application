...
$query = "SELECT SQL_CALC_FOUND_ROWS *  FROM ViewDailyPrice LIMIT ?, ?";
...
if (isset($_GET['filterscount']))
	...
	if ($filterscount > 0)
		...
		$filterquery.= "SELECT SQL_CALC_FOUND_ROWS * FROM ViewDailyPrice " . $where;
		// build the query.
		$valuesPrep = $valuesPrep . "ii";
		$values[] = & $start;
		$values[] = & $pagesize;
		$query = "SELECT SQL_CALC_FOUND_ROWS * FROM ViewDailyPrice " . $where . " LIMIT ?, ?";
		$result = $mysqli->prepare($query);
...
if (isset($_GET['sortdatafield']))
	...
	if ($sortorder != '')
		...
		if ($_GET['filterscount'] == 0)
			...
			if ($sortorder == "desc")
				{
				$query = "SELECT SQL_CALC_FOUND_ROWS * FROM ViewDailyPrice ORDER BY" . " " . $sortfield . " DESC LIMIT ?, ?";
				}
			  else if ($sortorder == "asc")
				{
				$query = "SELECT SQL_CALC_FOUND_ROWS * FROM ViewDailyPrice ORDER BY" . " " . $sortfield . " ASC LIMIT ?, ?";
				}
			$result = $mysqli->prepare($query);
			$result->bind_param('ii', $start, $pagesize);
			}
